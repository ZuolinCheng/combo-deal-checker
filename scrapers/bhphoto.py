...
def _parse_price(text: str) -> float:
    """Extract numeric price from a price string like '$749.00' or '1,249.99'."""
    if not text:
        return 0.0
...
def parse_bh_item(raw: dict) -> ComboDeal:
    """Parse a raw B&H Photo bundle dict into a ComboDeal model.
...
class BHPhotoScraper(BaseScraper):
    """Scraper for B&H Photo bundle/kit deals.

    Searches for bundles and kits across multiple queries.
    """
    ...
    async def scrape(self) -> list[ComboDeal]:
        """Search B&H Photo for bundle/kit deals."""
        seen_urls = set()
        deals = []

        for query in BHPHOTO_SEARCH_QUERIES:
            try:
        ...
        logger.info(
            ...
            f"from {len(BHPHOTO_SEARCH_QUERIES)} queries"
        )
        return deals

    async def _search_query(self, query: str) -> list[ComboDeal]:
        """Execute a single search query on B&H Photo."""
        search_url = (
    ...
    async def _extract_result(self, element) -> dict | None:
        ...
        if link_el:
            ...
            if href:
                url = (
                    f"https://www.bhphotovideo.com{href}"
                    if href.startswith("/")
                    else href
                )
